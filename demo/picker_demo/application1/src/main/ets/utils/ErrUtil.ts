...
export function tryFunc<R = void, P = never>(func?: (...params: P[]) => R, ...params: P[]): R {
  ...
  try {
    ...
    Log.error(TAG, `err: ${error?.message ?? error?.code ?? ''} ${func.name ? 'funcName: ' + func.name : ''}, params: ${params}`);
  }
  return res as R;
}
...
export async function tryWithErr<R, P = void>(func: Function, ...params: P[]): Promise<[R, undefined] | [undefined, Error]> {
  if (typeof func !== 'function') {
    Log.error(TAG, 'func.name, is not a func ' + typeof func + params);
    return undefined;
  }
  ...
  try {
    [res, err] = await func(...params).then((data: R): [R, undefined] => [data, undefined])
      .catch((error: Error): void => {
        Log.error(TAG, `tryWithErr err: ${error?.message ?? ''} ${func.name ? 'funcName: ' + func.name : ''}, params: ${params}`);
        err = error;
      });
  } catch (error) {
    Log.error(TAG, `err: ${error?.message ?? error?.code ?? ''} ${func.name ? 'funcName: ' + func.name : ''}, params: ${params}`);
